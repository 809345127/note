...
class GoImplToIntfCodeLensProvider {
    constructor(context) {
    ...
    resolveCodeLens(codeLens, _token) {
        ...
        const args = (_a = codeLens.command) === null || _a === void 0 ? void 0 : _a.arguments;
        if (!args || args.length < 2) {
            return null;
        }
        const lineNumber = args[0];
        const columnNumber = args[1];
    ...
    executeImplToIntfTool(filePath, lineNumber, columnNumber, showError = false) {
    ...
    // 检查方法是否实现了接口
    checkInterfaceWithTool(document, lineNumber, columnNumber) {
        return new Promise((resolve) => {
...
function activate(context) {
    // 注册命令
    let disposable = vscode.commands.registerCommand('go-impl-to-intf.findInterface', (lineNumber, columnNumber) => {
        // 获取当前活动编辑器
        const editor = vscode.window.activeTextEditor;
        if (!editor) {
            ...
            if (stdout) {
                ...
                if (jsonStart !== -1 && jsonEnd !== -1) {
                    ...
                    try {
                        ...
                        const options = {
                            selection: new vscode.Range(new vscode.Position(targetLine - 1, 0), new vscode.Position(targetLine - 1, 0)),
                            viewColumn: vscode.ViewColumn.Beside
                        };
                        vscode.window.showTextDocument(fileUri, options).then(() => {
                            // 如果找到了具体方法，显示更详细的信息
                            if (result.methodLine && result.method) {
                                vscode.window.showInformationMessage(`Found interface "${interfaceName}" with method "${result.method}" at line ${result.methodLine}`);
                            }
                            else {
                                vscode.window.showInformationMessage(`Found interface "${interfaceName}" at line ${line}`);
                            }
                        }, err => {
